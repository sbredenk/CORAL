...
def port_throughput(prs, df, region=None):
    if region:
        df = df.drop(columns=['index'])
        df = df[df['turbine_port'].isin(region)].reset_index(drop=True).reset_index()
    res = []
    ...
    legend_labels = {
        ...
        'portsmouth_1': 'Portsmouth',
        'tradepoint_1': 'Tradepoint Atlantic'
    }

    color_map = {
        'njwp_1': 'tab:blue',
        'sbmt': 'tab:orange',
        'new_london': 'tab:green',
        'new_bedford': 'tab:pink',
        'portsmouth_1': 'tab:purple',
        'tradepoint_1': 'tab:red'
    }
    ...
    ax = fig.add_subplot(111)
    throughput.plot.bar(ax=ax, width=0.75,color=[color_map.get(x, '#333333') for x in throughput.columns])
    ...
    name_updates_list = [legend_labels.get(label,label) for label in labels]
    # color_updates_list = [legend_colors.get(label,label) for label in labels]
    ax.legend(handles, name_updates_list, fontsize=6, ncol=5)
...
def compare_investments_bar(prs, us_rev, ffiv_rev, wtiv_rev, desc):
    ...
    for scenario in desc:
        # Getting cumulative value at 2040
...
def installed_cap(prs, dfs, desc, region = None):
    ...
    # labels = ['No Action', 'Infinite Resources']
    # labels = ['Base - no action', 'Base - infinite', 'No NJWP - no action', 'No NJWP - infinite']
    # labels = ['Base - no action', 'Base - infinite', 'Paulsboro - no action', 'Paulsboro - infinite']
    labels = ['Base - no action', 'Base - infinite', 'MD to NJWP - no action', 'MD to NJWP - infinite']
    # labels = ['Base - no action', 'No NJWP - no action', 'No NJWP + Paulsboro - no action']
    # labels = desc
    
    #ax.legend(labels)
    ax.legend(labels, prop={'size': 7}, loc=2)
    ax.set_ylim(top=60)
    ax.set_xlim([2023,2040])
    ax.set_xticks(np.arange(2025,2040,5))


    # ax.set_xlim(right=2040)
...
def compare_installed_cap(prs, dfs, desc, region=None):
    ...
    for df in dfs:
        ...
        df_2030 = pd.concat([df_2030, pd.DataFrame([row_2030])], ignore_index=True)
        df_2040 = pd.concat([df_2040, pd.DataFrame([row_2040])], ignore_index=True)
        df_2050 = pd.concat([df_2050, pd.DataFrame([row_2050])], ignore_index=True)
        i+=1
...
def installed_cap_region(prs, dfs, desc):
    ...
    for region_name, region_states in regions.items():
        ...
        # cum_label = [s + ' cumulative' for s in desc]
        # labels = ['Base', 'Congested', 'Expanded', 'Reduced', 'Uniform']
        # labels = ['Base - no action', 'Base - infinite', 'Paulsboro - no action', 'Paulsboro - infinite']
        ...
        # labels = cum_label
        if region_name in regional_targets:
            labels.append(f'{region_name} Targets')
        ax.legend(labels, prop={'size': 7}, loc=2)
...
def installed_cap_state(prs, dfs, desc):
    ...
    #These are the available state mandated procurement targets in Table 7 of the 2024 NREL OSW Market Report. That table also has "Planning Targets"
    state_targets = {
        'MA': {2035: 5.6},
        'ME': {2040: 3},
        'RI': {2030: 1.43},
    ...
    for state_name in states:
        ...
        df = dfs[0]
        df = df[df['location'] == state_name].reset_index(drop=True)
        ...
        df_cum_state[desc].plot(linestyle='-', ax=ax)

        if state_name in state_targets:
            state = state_targets[state_name]
            for year, capacity in state.items():
                ax.scatter(year, capacity, color='red', alpha=.5, label=f'{state_name} Targets')
                props = dict(boxstyle='square', facecolor='white', alpha=0.8, fill=True)
                ax.text(year+0.5, capacity-1, f"{capacity} GW", bbox=props)
        ...
        labels = desc
        if state_name in state_targets:
            labels.append(f'{state_name} Target')
        ax.legend(labels, prop={'size': 7})
        ax.set_xlim(right=2045)
        ax.set_ylim(top=12)

        slide_title = f'Cumulative Installed Capacity - {state_name}'
        slide = add_to_pptx(prs, slide_title)
...
def avg_delay(prs, dfs, desc, results_fp):
    ...
    for region_name, region_states in regions.items():
        ...
        slide = add_to_pptx(prs, slide_title)
        df_delay.to_csv(os.path.join(results_fp, f'avg_delay_{region_name}.csv'))
...
def run_plots(prs, df, ports):
    ...
    ports = ['njwp_1','tradepoint_1','sbmt','new_london']
    # ports = ['njwp_1','sbmt','new_london']
    # full_gantt(prs, df)
